...
## Overview
This feature restricts which form fields mobile users can edit in the **Pole Details** form. The restriction is enabled by default for mobile browsers and can be disabled **instantly without code changes or redeployment**.
...
- **Fallback**: Feature is **on by default for mobile**. Desktop users are unaffected.

## Default Behavior

Mobile browsers get restricted fields automatically unless the feature is explicitly disabled.
...
```javascript
localStorage.setItem('mobile_edit_restrictions', 'false');
console.log('Mobile edit restrictions disabled. Refresh the page.');
```
Then refresh the page.

### Method 2: JavaScript Snippet in Frontend
If you need to force-disable it temporarily in code, set:
```javascript
localStorage.setItem('mobile_edit_restrictions', 'false');
```
...
| Feature not turning off | Set `localStorage.mobile_edit_restrictions = 'false'`, then refresh. |
