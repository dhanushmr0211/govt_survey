...
export const PoleForm = ({ ulb, onBack }) => {
  const [formData, setFormData] = useState({
    ...
    arm_status: 'new',
    present_arm_no: '0',
    present_arm_length: '0',
    how_many_lights: '0',
    light_mounting_height: '5',
    light_type: 'bulb',
    light_capacity: '0W',
    light_working_status: 'yes',
    road_category: 'A1',
    road_type: 'MAIN ROAD',
    road_width: '4',
    pole_earthing_exists: 'YES',
  });
  ...
  const { data: switchPoints = [], refetch } = useQuery({
    queryKey: ['switchPoints', formData.ward_number],
    queryFn: async () => {
  ...
  useEffect(() => {
    if (switchPoints.length > 0) {
      setFormData((prev) => ({ ...prev, switch_point_id: switchPoints[0].id }));
    }
  }, [switchPoints]);
    ...
    <form onSubmit={handleSubmit} className="bg-white p-6 rounded-lg border border-gray-100 shadow-sm space-y-4 max-h-[70vh] overflow-y-auto">
      ...
      <div className="space-y-3 text-sm">
        ...
        <div>
          ...
          <select name="switch_point_id" value={formData.switch_point_id} onChange={handleChange} className="w-full p-2 border border-gray-200 rounded" required>
        ...
        <div>
          ...
          <select name="conductor_type" value={formData.conductor_type} onChange={handleChange} className="w-full p-2 border border-gray-200 rounded">
        ...
        <div>
          ...
          <select name="pole_type" value={formData.pole_type} onChange={handleChange} className="w-full p-2 border border-gray-200 rounded">
        ...
        <div>
          ...
          <select name="pole_height" value={formData.pole_height} onChange={handleChange} className="w-full p-2 border border-gray-200 rounded">
        ...
        <div>
          ...
          <select name="pole_condition" value={formData.pole_condition} onChange={handleChange} className="w-full p-2 border border-gray-200 rounded">
