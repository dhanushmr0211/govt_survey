export function SwitchPointInspectModal({ item, onClose }) {
  if (!item) return null;
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
      <div className="w-full max-w-lg rounded-xl bg-white p-5 shadow-xl">
        <div className="mb-4 flex items-center justify-between">
          <h3 className="text-lg font-bold text-slate-900">Inspect Switch Point</h3>
          <button onClick={onClose} className="text-sm font-semibold text-slate-500 hover:text-slate-700">Close</button>
        </div>

        <div className="grid grid-cols-2 gap-3 text-sm">
          <div>
            <p className="text-xs font-semibold uppercase text-slate-500">Switch Point Number</p>
            <p className="font-semibold text-slate-900">{item.switch_point_number || item.identifier || '-'}</p>
          </div>
          <div>
          ...
          <div>
            ...
            <p className="font-semibold text-slate-900">{item.ccms_number || '-'}</p>
          </div>
          <div>
