...
async function getSectionAccess(userId) {
  const result = await query('SELECT section_a, section_b, section_c, section_d, section_e, section_f, section_g, section_h FROM admin_section_access WHERE admin_id = $1', [userId]);
  return result.rows[0] || { section_a: false, section_b: false, section_c: false, section_d: false, section_e: false, section_f: false, section_g: false, section_h: false };
}
