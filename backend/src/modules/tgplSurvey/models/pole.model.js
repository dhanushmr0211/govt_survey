  ...
  const result = await query(
    `UPDATE poles 
     SET ward_id = COALESCE($3, ward_id),
         ...
         light_working_status = COALESCE($25, light_working_status),
         road_category = COALESCE($26, road_category),
         road_type = COALESCE($27, road_type),
         road_width_mtrs = COALESCE($28, road_width_mtrs),
         pole_earthing_exists = COALESCE($29, pole_earthing_exists),
         req_arm_number = COALESCE($30, req_arm_number),
         req_arm_length = COALESCE($31, req_arm_length),
         req_led_lights_no = COALESCE($32, req_led_lights_no),
         req_led_wattage = COALESCE($33, req_led_wattage),
         req_dedicated_wire = COALESCE($34, req_dedicated_wire),
         image_url_1 = COALESCE($35, image_url_1),
         image_url_2 = COALESCE($36, image_url_2),
         updated_at = NOW()
