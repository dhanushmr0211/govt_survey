...
async function updatePole(id, projectId, data) {
  const allowedFields = [
    'ward_id', 'ward_number', 'dtc_number', 'dtc_capacity', 'ccms_number',
    'meter_type', 'meter_rr_number', 'meter_serial_number', 'meter_dimensional_status',
    'conductor_type', 'pole_number', 'pole_type', 'pole_height', 'pole_to_pole_distance',
    'arm_type', 'arm_status', 'present_arm_no', 'present_arm_length', 'how_many_lights_in_pole',
    'light_mounting_height', 'light_type', 'light_capacity', 'light_type_2', 'light_capacity_2',
    'light_working_status', 'road_category', 'road_type', 'road_width_mtrs', 'pole_earthing_exists',
  ...
  if (setClauses.length === 0) {
    const existing = await query(
      `SELECT * FROM poles WHERE id = $1 AND project_id = $2 AND is_deleted = FALSE`,
      [id, projectId]
    );
